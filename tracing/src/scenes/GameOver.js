...
export class GameOver extends Scene {
    ...
    create() {
        ...
        const starburst = this.add.graphics({ x: width / 2, y: height / 2 });
        ...
        this.add.sprite(width / 2, height / 2, 'turtle_anim', 'turtle_3')
            .setScale(0.35 * scale)
            .setOrigin(0.5)
            .play('turtle_dance');

        const oldScore = this.score - this.earnedScore;
        const scoreContainer = this.add.container(width / 2, height / 2 - (180 * scale));
        ...
        // this.add.text(width / 2, height / 2 + (50 * scale), `Level Complete!\n\n${starsText}`, textStyle).setAlign('center').setOrigin(0.5);
        ...
        // this.add.text(width / 2, height / 2 + 250, continueText, smallStyle).setAlign('center').setOrigin(0.5);
        ...
        this.input.once('pointerdown', () => {
            ...
            if (isComplete) {
                ...
                this.scene.start(this.sceneName, { category: this.category, levelIndex: nextLevelIndex, score: this.score });
            }
